...
add_xls = common.get_xls("loan.xlsx", "downloadstatement")  #读取测试用例
...
class TestDownloadStatement(unittest.TestCase):
    def setParameters(self,case_name, method,  accountDate, merchantId,tradeType, resultCode, success):
        """
        set params
        :param case_name:
        :param method:
        ...
        # if accountDate != None and accountDate != '':
        #     self.accountDate = str(int(accountDate))
        # else:
        #     self.accountDate = None
        # if merchantId != None and merchantId != '':
        #     self.merchantId = str(int(merchantId))
        # else:
        #     self.merchantId = None
        # if tradeType != None and tradeType != '':
        #     self.tradeType = str(int(tradeType))
        # else:
        #     self.tradeType = None
    ...
    def tearDown(self):
        ...
        try:
            ...
            WeChatClient.sendmsg("DCCJ-签约结果查询用例","\nDCCJ-签约结果查询用例"+"\n用例名："+self.case_name+"\n响应时间(ms):"+str(self.time))
        except KeyError:
