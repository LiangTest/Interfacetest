...
add_xls = common.get_xls("loan.xlsx", "unbindCard")  #读取测试用例
...
class TestUnbindCard(unittest.TestCase):
    def setParameters(self, case_name, method, agreementNo,externalUserId,merchantId,productCode,resultCode, success):
        """
        set params
        :param case_name:
        :param method:
        ...
        if agreementNo != None and agreementNo !='':
            self.agreementNo = str(int(agreementNo))
        else:
    ...
    def tearDown(self):
        ...
        try:
            ...
            WeChatClient.sendmsg("DCCJ-解绑用例","\nDCCJ-解绑用例"+"\n用例名："+self.case_name+"\n响应时间(ms):"+str(self.time))
        except KeyError:
