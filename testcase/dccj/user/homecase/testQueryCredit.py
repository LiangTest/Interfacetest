...
add_xls = common.get_xls("loan.xlsx", "queryCredit")  #读取测试用例
...
class TestQueryCredit(unittest.TestCase):
    def setParameters(self,case_name, method, externalUserId, identifyNo, merchantId,productCode, resultCode, success):
        """
        set params
        :param case_name:
        :param method:
        ...
        if identifyNo != None and identifyNo !='':
            self.identifyNo = str(int(identifyNo))
        else:
            self.identifyNo = None
        if merchantId != None and merchantId !='':
            self.merchantId = str(int(merchantId))
        else:
            self.merchantId = None
        if productCode != None and productCode !='':
            self.productCode = str(int(productCode))
        else:
    ...
    def testQueryCredit(self):
        ...
        self.url = 'user/homeCase/queryCredit'
