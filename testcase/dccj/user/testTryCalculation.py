...
add_xls = common.get_xls("loan.xlsx", "tryCalculation")  #读取测试用例
...
class TestTryCalculation(unittest.TestCase):
    def setParameters(self,case_name, method,  loanId, merchantId,productId,repayType,source,stage, resultCode, success):
        """
        set params
        :param case_name:
        :param method:
        ...
        if productId != None and productId != '':
            ...
            self.productId = None
        if repayType != None and repayType != '':
            self.repayType = int(repayType)
        else:
            self.repayType = None
    ...
    def testPartialPayment(self):
        ...
        self.url = 'user/tryCalculation'
        configHttp.set_url(self.url)
        print("第一步：设置url  "+self.url)
        header = {}
        configHttp.set_header(header)
        print("第二步：设置header(token等)")
        params = {"loanId":self.loanId,"merchantId":self.merchantId,"productId":self.productId,"repayType":self.repayType,"source":self.source,"stage":self.stage}
    ...
    def tearDown(self):
        ...
        try:
            ...
            WeChatClient.sendmsg("DCCJ-试算用例","\nDCCJ-试算用例"+"\n用例名："+self.case_name+"\n响应时间(ms):"+str(self.time))
        except KeyError:
